...
# Czechia ------------------------------------------------------------------
d <- read.csv("Czechia.csv", header = TRUE, sep = ',')
head(d)
str(d)
summary(d)
...
# display results ----------------------------------------------------------
a <- plot_embeddings(data_result$results, "Czechia")
...
fig <- a + 
  ...
  theme(legend.text = element_text(size=rel(1.2))) + ggtitle("Czech Republic")
fig
...
ggsave(plot = figs,
       filename = "Czechia_method_comparison.tiff",
       device = "tiff", 
       path = "./",
       width = 18, 
       height = 8, 
